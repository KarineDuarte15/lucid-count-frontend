...
export default function CadastroEmpresaPage() {
  ...
  const handleConfirmarCadastro = async () => {
    ...
    try {
      ...
      router.push('/empresas'); // Redireciona para a lista de empresas após o sucesso
    } catch (err) {
       console.error(err);
  ...
  return (
    <main className="min-h-screen flex bg-primary-card">
        <Sidebar /> {/* Sidebar integrado aqui */}
        
        <div className="flex-1 flex flex-col">
            <header className="flex justify-between items-center p-4 bg-white text-gray-800 shadow-md">
                <h1 className="text-xl font-bold">
                    {step === 1 ? 'Cadastro de Nova Empresa' : 'Revisão do Cadastro'}
                </h1>
            </header>
                                ...
                                <div className="p-4 border rounded-lg bg-white shadow-sm">
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                        ...
                                        <div className="md:col-span-1 grid grid-cols-3 gap-2">
                                            <input type="text" placeholder="Cidade" name="cidade" value={empresaData.endereco?.cidade} onChange={handleEnderecoChange} className="col-span-2 p-2 border rounded-md" />
                                            <input type="text" placeholder="UF" name="uf" value={empresaData.endereco?.uf} onChange={handleEnderecoChange} className="p-2 border rounded-md" />
                                        </div>
